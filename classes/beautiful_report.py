...
class MakeResultJson:
    ...
    def __repr__(self) -> str:
        ...
        for key, data in zip(keys, self.data):
...
class ReportTestResult(unittest.TestResult):
    ...
    def startTest(self, test) -> None:
        ...
        stdout_redirect.fp_msg = self.output_buffer
        stderr_redirect.fp_msg = self.output_buffer
    ...
    def get_all_result_info_tuple(self, test) -> tuple:
        ...
        :return:
        """
        return tuple([*self.get_test_case_property(test), self.end_time, self.status, self.case_log])
    ...
    def addError(self, test, err):
        """
         add Some Error Result and info
        :param test:
        :param err:
        :return:
        """
    ...
    def addFailure(self, test, err):
        """
        add Some Failures Result and info
        :param test:
        :param err:
        :return:
        """
    ...
    @staticmethod
    def get_test_case_property(test) -> tuple:
        """
            ...
            def __wrap(*args, **kwargs):
                ...
                try:
                    ...
                    if 'save_img' in dir(args[0]):
                        ...
                        data = BeautifulReport.img2base(img_path, params[0] + '.png')
                ...
                if len(params) > 1:
                    for param in params:
                        print(param + ':')
                        data = BeautifulReport.img2base(img_path, param + '.png')
                        print(HTML_IMG_TEMPLATE.format(data, data))
                    return result
                if not os.path.exists(img_path + params[0] + '.png'):
                    return result
                data = BeautifulReport.img2base(img_path, params[0] + '.png')
                print(HTML_IMG_TEMPLATE.format(data, data))
                return result
